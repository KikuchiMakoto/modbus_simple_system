...
**KiCADやArduino IDEは、深く理解しなくて大丈夫です。** 発注データはできあがっていますし、書き込みもスクリプト1本で終わります。ここは多少「なあなあ」でも実験は進みます。

その代わり、**センサー周り（[LoadCell](./LoadCell.md)・[LVDT](./LVDT.md)・[HX711](./HX711.md)・[ADS1115](./ADS1115.md)・[歪みゲージ](./StrainGauge.md)・[励振](./Excitation.md)）だけはよく調べてください。** センサーこそがあなたの実験本体であり、測定値の精度そのものです。基板やコードは「センサーの値を運ぶ手段」でしかなく、センサーの選定・配線・校正を間違えれば、どれだけ基板やソフトが正しくても出てくる数字は意味を持ちません。

計測はセンサーありきです。時間配分に迷ったら、まずセンサーのドキュメントを読み込むことに使ってください。
